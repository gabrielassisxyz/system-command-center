...
- `newTestAssembler` is typed against `metrics.SystemSource`, so a test that wants a completely empty source can set `fakeSystemSource{absent: true}`; this avoids passing a different concrete type to the helper and keeps the assembler boundary exercised by the same fake. Also, the rate collectors must deep-copy returned maps/slices before storing them as the previous sample; otherwise mutating a fake source's data between calls corrupts the baseline and produces zero rates on the second assembly.
